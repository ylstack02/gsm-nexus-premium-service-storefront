  ...
  const NavLink = ({ to, children }: { to: string, children: React.ReactNode }) => {
    ...
    return (
      <Link 
        to={to} 
        className={cn(
          "text-sm font-bold uppercase tracking-widest transition-all relative py-1",
          ...
          <div className="flex items-center gap-3">
            <form onSubmit={handleSearch} className="hidden sm:block relative">
              ...
              <input
                ...
                className="h-10 w-48 lg:w-64 rounded-full bg-slate-100 dark:bg-slate-800 pl-9 pr-4 text-xs font-medium focus:outline-none focus:ring-2 focus:ring-cyan-500/10 transition-all border border-transparent focus:bg-background focus:border-cyan-500/30"
              />
            </form>
                ...
                <SheetContent side="right" className="w-[300px]">
                  ...
                  <div className="flex flex-col gap-3">
                    <Link
                      ...
                      className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 font-bold text-sm uppercase tracking-widest"
                    >
                      Browse Catalog
                      <Badge variant="outline" className="border-cyan-500 text-cyan-500">25+</Badge>
                    </Link>
                    <Link
                      to="/track"
                      onClick={() => setMobileMenuOpen(false)}
                      className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 font-bold text-sm uppercase tracking-widest"
                    >
                      Track Order
                    </Link>
                    <Link
                      to="/checkout"
                      onClick={() => setMobileMenuOpen(false)}
                      className="p-5 rounded-2xl bg-cyan-500 text-white font-bold flex justify-between items-center text-sm uppercase tracking-widest shadow-lg shadow-cyan-500/20 mt-4"
                    >
                      Shopping Cart
                      <span className="bg-white/20 px-3 py-1 rounded-full text-xs">{cartCount}</span>
                    </Link>
                  </div>
                </SheetContent>
              </Sheet>
